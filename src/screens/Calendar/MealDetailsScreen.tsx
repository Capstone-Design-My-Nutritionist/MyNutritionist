...
import { getMealRecordByDateAndType, dummyMealRecord } from '../../data/dummyMealRecordData';
...
const MealDetailsScreen = () => {
  ...
  const { date, mealType = '점심' } = route.params || {};
  
  // 식사 기록 데이터 가져오기 (날짜와 식사 타입으로)
  const mealRecord = getMealRecordByDateAndType(date, mealType);
  
  // 데이터가 없을 경우 기본 더미 데이터 사용
  const mealData = mealRecord ? {
    ...
    date: date || dummyMealRecord.date,
    mealType: mealType || dummyMealRecord.mealType,
